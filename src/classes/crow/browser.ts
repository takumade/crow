import { Builder, By, Key, TouchSequence, until, WebDriver } from 'selenium-webdriver';
import fs from 'fs'
...
export class Browser {
    ...
    async scrollScreen(){
        await this.syncExecuteJS("window.scrollTo(0,document.body.scrollHeight)")
        await this.sleep(1000, 3000)
    }

    async finishedScrolling(threshold:number = 97){
        return await this.syncExecuteJS(`return scrollY/document.body.scrollHeight * 100 >= ${threshold}`)
    }
